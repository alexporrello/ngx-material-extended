...
export declare interface NgxEditableTdEvent {
    value: string;
    oldValue: string;
...
export const NgxEditableTdValidator = {
    number: /^[0-9]+([.]*[0-9]+)*$/
};

@Component({
    selector: 'ngx-editable-td',
    templateUrl: './ngx-editable-td.html',
    providers: [{ provide: NgxCell, useExisting: NgxEditableTd }],
    host: {
...
export class NgxEditableTd extends NgxCell implements OnDestroy {
    ...
    public readonly validator = input<keyof typeof NgxEditableTdValidator>();
    ...
    private readonly _validator = computed(() => {
        ...
        if (builtIn) return NgxEditableTdValidator[builtIn];
